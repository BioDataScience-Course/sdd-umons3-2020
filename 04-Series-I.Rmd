...
```{block2, type = 'warning'}
La fonction `trend.test()` de {pastecs} version < 1.4 est incompatible avec le tidyverse et avec l'instruction `SciViews::R`. La SciViews Box jusqu'à sa version 2020 contient la version 1.3 de {pastecs} par défaut. Il faut donc forcer la mise-à-jour avant de pouvoir réaliser l'analyse de test de tendance via bootstrap. Cela se fait facilement avec l'instruction suivante à la console R\ : `remotes::install_github("phgrosjean/pastecs@v1.4.0", upgrade = FALSE)`. Cette opération ne doit être réalisée qu"une seule fois, mais si le package est déjà chargé en mémoire avant la mise-à-jour, il faut redémarrer R pour être certain d'utiliser la dernière version (menu RStudio `Session -> Restart R`). Pour vérifier la version utilisée, on peut faire\ : `packageVersion("pastecs")`.
```
...
![Utilisation de `identify()` avec un graphique généré à la Console R pour déterminer les points de transitions des segments de droites représentant les sommes cumulées.](images/04-series1/identify_local_trend.gif)
...
if (exists("assignation"))
  assignation("C04Ga_ts", part = "I",
    ...
    toc = "Analyse de séries spatio-temporelles")
```
