...
Ce jeu de données comprend `r ncol(penguins)` variables et `nrow(penguins)` individus. On observe la présence de quelques données manquantes que nous supprimons avant de continuer. Nous supprimons également la variable année qui ne nous intéresse pas.

```{r}
penguins %>.%
  drop_na(penguins) %>.%
  select(., -year) -> penguins
```
...
```{r}
chart(penguins, bill_length_mm ~ flipper_length_mm %color=% species) +
  geom_point() +
  labs(y = "Longueur du bec [mm]", x = "Longueur des nageoires [mm]", color = "Espèces")
```
...
L'exemple proposé précédemment est volontairement très simple afin de comprendre les principes de base de la classification supervisée avec un algorithme simple.
