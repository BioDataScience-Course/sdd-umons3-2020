...
```{r}
# idee de l'exerice de présentation
SciViews::R
library(mlearning)


zoo <- data.io::read("zooplankton", package = "data.io")
vec <- c("Decapod", "Malacostracan", "Poecilostomatoid")

zoo %>.%
  filter(., class %in% vec) %>.%
  mutate(., class = droplevels(class)) -> zoo_red

n <- nrow(zoo_red)
n_learning <- round(n * 7/10)
set.seed(164)
learning <- sample(1:n, n_learning)
zoo_learn <- zoo_red[learning, ]
zoo_test <- zoo_red[-learning, ]

zoo_lda <- lda(
  formula = class ~ .,
  data = zoo_learn)

zoo_pred <- predict(zoo_lda, zoo_test)$class

zoo_conf <- confusion(zoo_pred, zoo_test$class)
