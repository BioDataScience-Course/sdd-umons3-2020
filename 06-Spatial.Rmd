...
Un objet **sf** est tout simplement un tableau de données classiques de type cas par variables que vous avez l'habitude de manipuler, mais qui contient une colonne décrivant un polygone pour la zone géographique concernée (objet **sfc_MULTIPOLYGON** ici dans la colonne `geometry`).
...
Dans la version précédente, nous indiquions tous les esthétiques directement dans la formule fournie à `chart()`. Mais nous pouvons également les fournir individuellement à chaque appel de `geom_sf()`. Il faut alors les fournir via `aes()`. Cette méthode est même beaucoup plus compréhensible lorsque nous allons superposer différentes couches sur la carte. AZinsi, le code suivant donne exactement le même résultat.
...
La même carte n'est pas réalisable avec {ggplot2}/`chart()` parce que le modèle terrain contient plus de 3,7 milliuons de points et que c'est beaucoup trop de données pour ce moteur graphique-là. Néanmoins, nous pouvons réaliser une carte en utilisant {tmap} à conditions de convertir nos objets d'abord. Pour le modèle terrain, nous faisons\ :
...
```{r}
mbord[[1]] <- mbord[[1]][-(1:28), ] # Useless adata at the beginning
...
Enfuin, nous réalisons une carte de la pluviométrie que nous plaçons côte à côte avec la carte du releif pour comparaison.
